...
### MINIMAL COST & FREE-FIRST DEFAULT (FOUNDATIONAL)
- Treat **minimizing spend and usage** as a standing constraint: Cursor/agent turns, remote Cloud agents, paid APIs, and unnecessary cloud resources should stay **as low as practical** unless the user clearly opts into higher cost.
- Prefer **local / Worktree** execution over **Cloud** agents when both can satisfy the task; use Cloud mainly for async work or when the machine cannot run the job.
- Minimize tool churn: batch reads, avoid redundant searches and parallel subagents when a single pass suffices, reuse context already in the thread.
- Do **not** add paid third-party services, billable APIs, or quota-heavy integrations without **explicit** user approval.
- Align with concise communication (see `concise.mdc`): shorter, denser outputs reduce back-and-forth cost.
